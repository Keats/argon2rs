...
pub type Block = [u64x2; per_block!(u64x2)];

fn zero() -> Block { [u64x2(0, 0); per_block!(u64x2)] }

fn xor_all(blocks: &Vec<&Block>) -> Block {
    let mut rv: Block = zero();
    for (idx, d) in rv.iter_mut().enumerate() {
        *d = blocks.iter().fold(u64x2(0, 0), |n, &&blk| n ^ blk[idx]);
    }
    rv
}
...
fn as_u64_mut(b: &mut Block) -> &mut [u64] {
    let rv: &mut [u64; per_block!(u64)] = unsafe { mem::transmute(b) };
    rv
}

fn as_u64(b: &Block) -> &[u64] {
    let rv: &[u64; per_block!(u64)] = unsafe { mem::transmute(b) };
    rv
}
...
impl Gen2i {
    ...
    fn new(start_at: usize, pass: u32, lane: u32, slice: u32, totblocks: u32,
        ...
        for (k, v) in as_u64_mut(&mut rv.arg).iter_mut().zip(args.into_iter()) {
            *k = *v as u64;
        }
        rv.more();
        rv
    ...
    fn nextj(&mut self) -> (u32, u32) {
        let rv = {
            let u64block = as_u64(&self.pseudos);
            split_u64(u64block[self.idx])
        };
...
macro_rules! p_u64x2 {
    ($v0v1: expr, $v2v3: expr, $v4v5: expr, $v6v7: expr,
     $v8v9: expr, $v10v11: expr, $v12v13: expr, $v14v15: expr) => {
        {
            g_blake2b_u64x2!($v0v1, $v4v5, $v8v9, $v12v13);
            g_blake2b_u64x2!($v2v3, $v6v7, $v10v11, $v14v15);
            let (mut v7v4, mut v5v6) = $v4v5.cross_swap($v6v7);
            let (mut v15v12, mut v13v14) = $v12v13.cross_swap($v14v15);
            g_blake2b_u64x2!($v0v1, v5v6, $v10v11, v15v12);
            g_blake2b_u64x2!($v2v3, v7v4, $v8v9, v13v14);
...
pub fn p_row(row: usize, b: &mut Block) {
    p_u64x2!(b[8 * row + 0],
             b[8 * row + 1],
             b[8 * row + 2],
             b[8 * row + 3],
...
pub fn p_col(col: usize, b: &mut Block) {
    p_u64x2!(b[8 * 0 + col],
             b[8 * 1 + col],
             b[8 * 2 + col],
             b[8 * 3 + col],
...
mod kat_tests {
    ...
    pub fn u8info(prefix: &str, bytes: &[u8], print_length: bool) -> String {
        let bs = bytes.iter()
    ...
    pub fn block_info(i: usize, b: &[u64]) -> String {
    ...
    fn gen_kat(a: &mut super::Argon2, tau: u32, p: &[u8], s: &[u8], k: &[u8],
        ...
        for (i, block) in a.blocks.iter().enumerate() {
            rv = rv + &block_info(i, super::as_u64(block));
        }
